...
When both services are running, navigate your browser to the http://localhost:9081/inventory/systems/localhost/properties URL. This triggers an HTTP GET request to the `inventory` service, which then makes an outbound GET request to the `/system/properties` endpoint of the `system` service. If the request succeeds, the system `localhost` is stored in the inventory. In addition, the `inventory` service automatically pings the `system` service every 30 seconds to refresh the health status of all systems currently tracked in the inventory.

To view telemetry data, open the Grafana dashboard at the http://localhost:3000 URL and follow these steps:
...
4. Click **Run query** at the upper-right corner to list recent traces.

5. Click a trace ID with the name `GET /inventory/systems/{hostname}/properties` that you generated. You see the following result:

+
image::inventory_default_spans.png[`GET /inventory/systems/{hostname}/properties` trace,align="center"]

+
The trace contains three spans, one for the initial request to the `inventory` endpoint, one client span from the `inventory` service making outbound call to the `system` service, and one server span from the `system` service handling that request. Under **Service & Operation**, you can view these spans and click each one to inspect details such as when the request was received and when the response was sent, along with metadata like HTTP method, status code, and endpoint path.

6. Expand the **Node graph** to see the relationship of spans between the `inventory` and `system` microservices.

+
image::inventory_node_graph.png[`GET /inventory/systems/{hostname}/properties` node graph,align="center"]
...
1. Open the **Drilldown -> Logs** view from the left menu. This view displays an overview of time series and log visualizations for all services that send logs to Loki.
...
Currently, the [hotspot file=0]`SystemClient` class logs messages by using [hotspot=out3 hotspot=out4 hotspot=out8 file=0]`System.out` and [hotspot=out1 hotspot=out2 hotspot=out5 hotspot=out6 hotspot=out7 file=0]`System.err`.

To observe a basic standard output log, open your browser and navigate to the http://localhost:9081/inventory/systems/localhost/properties URL. Then, open the Grafana dashboard at the http://localhost:3000 URL.
...
Return to the http://localhost:9081/inventory/systems/localhost/properties URL and rerun the Loki query in Grafana. This time, the log is structured. The `detected_level` is set to `INFO`, and the `io_openliberty_module` field contains the logger name, making it easier to trace the origin of the log.

image::log_logger_ok.png[Log by using the java.util.logging API,align="center"]
{empty} +

Access the http://localhost:9081/inventory/systems/unknown/properties URL again to simulate an exception, then rerun the Loki query in Grafana. The resulting log shows `WARNING` in the `detected_level` field, includes a structured stack trace in the `exception_stacktrace` field, and identifies the exception type as `jakarta.ws.rs.ProcessingException` in the `exception_type` field.
