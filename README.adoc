...
Because the `inventory` service makes periodic background calls every 15 seconds to refresh system load information for all tracked systems, telemetry data is continuously generated for you to monitor.
...
Select the **Logs** view at the upper right of the **Logs** section to show the results in logs visualization, which enables log expansion.
...
Now, access the http://localhost:9081/inventory/systems/unknown URL to simulate a request to a nonexistent host, which triggers a `RuntimeException`.
