...
:page-related-guides: [ 'microprofile-telemetry-grafana-custom', 'microprofile-telemetry-jaeger', 'microprofile-metrics' ]
...
The diagram shows a distributed environment with multiple services. For simplicity, this guide configures only the `system` and `inventory` services. You’ll learn how to enable automatic collection of traces, metrics, and logs using MicroProfile Telemetry, and visualize them in Grafana.
...
Similarly, the added [hotspot=microprofile hotspot=mpTelemetry file=1]`mpTelemetry` feature enables telemetry support for the `system` service.
...
Next, interact with the services to generate request data that provides richer insights into the telemetry.

The `system` microservice simulates a service that returns system load information for a host. You can access the `localhost` system load information at the http://localhost:9080/system/systemLoad URL.

The `inventory` microservice requests system load information from the `system` service and stores it. To fetch and store the `localhost` system information, visit the http://localhost:9081/inventory/systems/localhost URL.
