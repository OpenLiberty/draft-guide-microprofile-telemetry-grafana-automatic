...
The diagram shows a distributed environment with multiple services. For simplicity, this guide configures only the `system` and `inventory` services. The services use Jakarta RESTful Web Services (JAX-RS) and MicroProfile REST Client to handle inbound and outbound requests. You’ll learn how to enable automatic collection of traces, metrics, and logs from microservices by using MicroProfile Telemetry, and visualize them in Grafana.
...
OpenTelemetry automatically generates trace spans for incoming HTTP requests to Jakarta RESTful Web Services endpoints and outgoing requests from MicroProfile REST Clients. It also collects metrics such as HTTP request durations and JVM performance, and captures logs from both the server and the application.
...
Telemetry such as Liberty startup logs and JVM metrics is generated on startup. To see request-scoped telemetry, interact with the services.
...
----
include::start/inventory/src/main/java/io/openliberty/guides/inventory/InventoryManager.java[]
----
...
The updated [hotspot file=1]`InventoryManager` class now uses the [hotspot=getLogger file=1]`Logger.getLogger()` method to obtain a logger instance and [hotspot=log1 hotspot=log2 hotspot=log3 file=1]`Logger.log()` method to write messages at appropriate levels, such as [hotspot=log1 file=1]`INFO` for successful operations and [hotspot=log2 hotspot=log3 file=1]`WARNING` for exceptions.
