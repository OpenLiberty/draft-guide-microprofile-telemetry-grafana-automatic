...
:page-related-guides: [ 'microprofile-telemetry-grafana-custom', 'microprofile-telemetry-jaeger', 'microprofile-metrics', 'microprofile-health' ]
:imagesdir: /img/guide/{projectid}
:page-seo-title: Collecting automatic traces, metrics, and logs in Java microservices with MicroProfile Telemetry and Grafana
...
When both services are running, visit the http://localhost:9081/inventory/systems/localhost URL to register the `localhost` system in the inventory.

The `inventory` service makes periodic calls to the `system` service every 15 seconds to refresh system load information for all tracked systems.

To view the telemetry data generated, open the Grafana dashboard at the http://localhost:3000 URL and follow these steps:
...
2. Review the log entries displayed for each service. You can click *Show logs* for a service to display its log entries inline and expand a log entry to view the full trace context.

+
image::log_logger_info.png[Example of log details,align="center"]
...
Similarly, the added [hotspot=mpTelemetry file=1]`mpTelemetry` feature enables telemetry support for the `system` service.
...
The [hotspot=otel file=3]`otel.*` properties are configured in the [hotspot file=3]`bootstrap.properties` file for the `system` service to enable telemetry collection and define service-specific settings.
...
By default, OpenTelemetry generates trace spans for incoming HTTP requests to JAX-RS and REST endpoints, collects metrics such as HTTP request durations and JVM performance, and captures message logs written at the `INFO` level or higher.
...
When both services are running, visit the http://localhost:9081/inventory/systems/localhost URL to register the `localhost` system in the inventory. This action generates default telemetry data for the request.

Because the `inventory` service makes periodic background calls every 15 seconds to refresh system load information for all tracked systems, telemetry data will be continuously generated for you to monitor.
...
6. In the **Service & Operation** table, click each span to view detailed metadata. This includes when the request was received and when the response was sent, as well as information such as the HTTP method, status code, and endpoint path.

+
image::inventory_systems_localhost_spans.png[`GET /inventory/systems/{hostname}` spans,align="center"]
...
+
image::inventory_systems_localhost_node_graph.png[`GET /inventory/systems/{hostname}` node graph,align="center"]
...
Monitor timestamped logs generated by the services since the server startup:

1. Navigate to **Drilldown -> Logs** from the menu. his view displays a time-series overview of log counts and log entries for all services.

+
image::logs_overview.png[Logs overview,align="center"]
...
Currently, the [hotspot file=0]`SystemClient` class logs messages by using [hotspot=out3 file=0]`System.out` and [hotspot=out1 hotspot=out2 hotspot=out4 hotspot=out5 hotspot=out6 hotspot=out7 file=0]`System.err`.
...
In the **Explore** view, select the **Loki** data source. Set a filter for `service_name = inventory` and click the blue **Run query** button.
...
image::logs_view.png[Logs view,align="center"]

Locate the log entry `Received response with status: 200`. When expanded, you see that both the `detected_level` and the `io_openliberty_module` fields are set to `SystemOut`.

image::log_system_out.png[Example log entry from `System.out` standard output stream,align="center"]
{empty} +

Now, access the http://localhost:9081/inventory/systems/unknown URL to simulate a request to a nonexistent hostwhich triggers a `RuntimeException`.
...
This log entry shows `SystemErr` as the value for both the `detected_level` and `io_openliberty_module` fields. The error stack trace is included directly in the log message, and no information about the exception type is provided.

image::log_system_err.png[Example log entry from `System.err` standard error stream,align="center"]
{empty} +

These examples show that while `System.out` and `System.err` logs are collected, they do not provide the structured metadata needed to filter by severity, separate stack traces, or correlate logs with originating classes or error types.
...
The updated [hotspot file=1]`SystemClient` class uses the [hotspot=getLogger file=1]`Logger.getLogger()` method to retrieve a logger instance and the [hotspot=log1 hotspot=log2 hotspot=log3 hotspot=log4 hotspot=log5 hotspot=log6 hotspot=log7 file=1]`Logger.log()` method to emit log messages at specific levels such as [hotspot=log3 file=1]`INFO` or [hotspot=log1 hotspot=log2 hotspot=log4 hotspot=log5 hotspot=log6 hotspot=log7 file=1]`WARNING` based on the context.

Because you are running the services in dev mode, the changes that you made are automatically picked up.

Return to the http://localhost:9081/inventory/systems/localhost URL to trigger a successful request. Rerun the Loki query in Grafana and locate the log entry `Received response with status: 200`. Expand the entry and verify that the `detected_level` is set to `INFO` and the `io_openliberty_module` field contains the logger name, `io.openliberty.guides.inventory.client.SystemClient`, which helps trace the origin of the log.

image::log_logger_info.png[Example log entry from java.util.logging API at INFO level,align="center"]
{empty} +

Access the http://localhost:9081/inventory/systems/unknown URL to trigger an exception. Rerun the Loki query in Grafana and locate the log entry `Runtime exception while invoking system service`. Expand it and verify that the `detected_level` field is set to `WARNING`. The `exception_stacktrace` field displays a structured stack trace, and the `exception_type` field identifies the exception as `jakarta.ws.rs.ProcessingException`.

image::log_logger_warning.png[Example log entry from java.util.logging API at WARNING level,align="center"]
{empty} +

By default, OpenTelemetry collects only message logs. For details on how to include other sources, see https://openliberty.io/docs/latest/reference/feature/mpTelemetry-2.0.html#logs[Collect logs from a specified source^].
