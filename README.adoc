...
The diagram shows a distributed environment with multiple services. For simplicity, this guide configures only the `system` and `inventory` services. The `system` service provides system load information, while the `inventory` service retrieves and stores this data by calling the `system` service through a MicroProfile REST Client. Both services expose endpoints built with Jakarta RESTful Web Services.
...
This guide demonstrates automatic tracing using MicroProfile REST Client on the client side. If you want to see how distributed tracing works with Jakarta REST Clients instead, see the https://openliberty.io/guides/microprofile-telemetry-jaeger.html[Enabling distributed tracing in microservices with OpenTelemetry and Jaeger^] guide.
...
1. From the **Trace** view you opened in the previous step, click the image:log_icon.svg[log icon, 18, 18] **Log icon** on the right side of the span entry in the **Service & Operation** table.
